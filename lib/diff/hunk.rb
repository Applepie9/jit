  ...
  Hunk = Struct.new(:a_start, :b_start, :edits) do
    def self.filter(edits)
      ...
      loop do
        ...
        a_start = offset.negative? ? 0 : edits[offset].a_line.number
        b_start = offset.negative? ? 0 : edits[offset].b_line.number
    ...
    def self.build(hunk, edits, offset)
      ...
      until counter.zero?
        hunk.edits.push(edits[offset]) if (offset >= 0) && counter.positive?
