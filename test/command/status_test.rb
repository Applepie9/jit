...
describe Command::Status do
  ...
  describe "status command with porcelain option" do
    def assert_status_porcelain(output)
    ...
    it "lists files as untracked if they are not in the index" do
      ...
      assert_status_porcelain <<~STATUS
        ?? file.txt
      STATUS
    end
  ...
  describe "status command without arguments" do
    ...
    it "does not list empty untracked directories" do
      ...
      assert_status "nothing to commit, working tree clean\n"
    end
